...
export function ClientPortalSection({
  ...
  useEffect(() => {
    if (inviteCode || fetchingRef.current) return
  ...
  function getShareUrl() {
    ...
    return `${window.location.origin}/share/${inviteCode}`
  }
  ...
  async function handleSendEmail() {
    ...
    setError(null)
    setSent(false)
    const endpoint = onboardedAt ? '/api/v1/portal/send-login-link' : '/api/v1/invites'
    ...
    <div className="rounded-lg border border-border-subtle bg-surface-raised p-5">
      ...
      {inviteCode ? (
        <div className="space-y-3">
          ...
          >
            {sending ? 'Sending...' : sent ? 'Email sent' : onboardedAt ? 'Send portal link via email' : 'Send invite email'}
          </button>
          ...
          {!clientEmail && (
            <p className="text-xs text-text-tertiary">Add an email to send links.</p>
          )}
        </div>
      ) : (
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 py-1">
            ...
            <span className="text-xs text-text-tertiary">Generating share link...</span>
          </div>
        </div>
      )}
