        ...
        {sorted.map((stage, i) => {
          ...
          return (
            <div key={stage.id} className="flex items-center" role="listitem">
              {i > 0 && (
                <div className={`w-4 h-px ${isCompleted || isCurrent ? 'bg-accent/40' : 'bg-text-tertiary/40'}`} />
              )}
              <div
            ...
            {i > 0 && (
              <div
                className={`absolute top-[7px] right-1/2 w-full h-px ${
                  isCompleted || isCurrent ? 'bg-accent/40' : 'bg-text-tertiary/40'
                }`}
              />
            )}
