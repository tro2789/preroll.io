...
export function DeliverableCard({ deliverable, episodeContext, reviewUrl, thumbnailUrl: initialThumb, allowDownload }: DeliverableCardProps) {
  ...
  return (
    <div className="rounded-lg bg-surface-raised border border-border-subtle overflow-hidden">
      <div className="flex">
        {thumb && !thumbFailed && (
          <div className="shrink-0 w-48 relative overflow-hidden">
            {reviewUrl ? (
              <a href={reviewUrl} className="block h-full relative group/thumb">
                <img src={thumb} alt="" className="absolute inset-0 w-full h-full object-cover" onError={() => setThumbFailed(true)} />
        ...
        <div className="flex-1 min-w-0 p-4 space-y-3">
          ...
          {!isPending && (reviewUrl || (allowDownload && deliverable.file_url)) && (
            <div className="flex items-center gap-2 pt-1">
              {reviewUrl && (
                <a
                  href={reviewUrl}
                  className="inline-flex items-center gap-1.5 rounded-md border border-border-default bg-surface-overlay px-3 py-1.5 text-xs font-medium text-text-primary hover:bg-surface-input transition-colors"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="w-3 h-3">
                    <path d="M3.05 3.05a7 7 0 1 1 9.9 9.9 7 7 0 0 1-9.9-9.9Zm1.627 8.273A5.5 5.5 0 1 0 12.323 4.677L4.677 12.323ZM6.75 6a.75.75 0 0 0-.75.75v2.5a.75.75 0 0 0 1.105.66l2.255-1.25a.75.75 0 0 0 0-1.32l-2.255-1.25A.75.75 0 0 0 6.75 6Z" />
                  </svg>
