...
The Vimeo integration enables video hosting and delivery for your podcast episodes. Upload finished video episodes, generate private review links, and embed videos in your client portal — all managed from within PreRoll.
...
3. Authorize PreRoll to access your Vimeo account
...
PreRoll organizes videos within Vimeo projects (folders):
...
PreRoll uses the **tus resumable upload protocol** for reliable video uploads to Vimeo:
...
1. PreRoll creates a video resource on Vimeo with metadata
...
5. PreRoll receives a webhook when processing completes
...
Once a video is uploaded and processed, PreRoll can generate private review links:
...
Review links are automatically attached to the corresponding deliverable in PreRoll.

## Embedding Videos

Videos hosted on Vimeo can be embedded directly in the PreRoll client portal:
...
| Embed not loading | Verify domain whitelist includes your PreRoll domain |
| Quota exceeded | Check your Vimeo plan's weekly upload limit |
| Token expired | PreRoll automatically refreshes tokens — reconnect if issues persist |
