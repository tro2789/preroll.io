...
3. In PreRoll, navigate to the show's **Settings → Distribution**
4. Paste your Transistor API key and select which Transistor podcast to link

Each show in PreRoll connects to one Transistor podcast. If you manage multiple podcasts on Transistor, use the same API key — it grants access to all podcasts on your account.

## Connecting a Show

1. Open a show in PreRoll
2. Go to **Settings → Distribution**
3. Enter your Transistor API key
4. PreRoll fetches your Transistor podcasts — select the one to link
...
- Upload progress is tracked in PreRoll
...
Scheduled episodes show a countdown in both PreRoll and Transistor's dashboard.

## Checking Publish Status

PreRoll tracks the publish status of each episode:
