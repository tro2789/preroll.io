...
The Frame.io integration enables video review workflows directly within PreRoll. Upload cuts for client review, receive timecoded feedback, and sync approval status — all without leaving your production pipeline.
...
3. You'll be redirected to Adobe's login page to authorize PreRoll
...
When you create a deliverable or initiate a review, PreRoll can automatically create a Frame.io project structure:
...
Comments sync bidirectionally between PreRoll and Frame.io:

| Direction | Behavior |
|-----------|----------|
| PreRoll → Frame.io | Timecoded comments added in the PreRoll review player are pushed to the Frame.io asset |
| Frame.io → PreRoll | Comments added directly in Frame.io are pulled into the PreRoll episode timeline |

Sync happens via Frame.io webhooks. When a comment is created or updated on a linked asset, PreRoll receives the notification and updates the local comment thread.
...
Frame.io sends webhook events to PreRoll for:
...
| Token expired | PreRoll automatically refreshes tokens — if issues persist, reconnect the integration |
