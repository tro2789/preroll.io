...
```
PreRoll Comment → Frame.io Comment (on linked asset)
Frame.io Comment → PreRoll Comment (via webhook)
```
...
Once linked, any comment left in PreRoll appears in Frame.io (and vice versa), preserving timestamps.
