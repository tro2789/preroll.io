---
title: "FAQ"
description: "Frequently asked questions about PreRoll — billing, limits, security, integrations, and more."
---
...
PreRoll offers three plans:

| Plan | Price | Best for |
|------|-------|----------|
| Free | $0/month | Trying PreRoll with a single client |
...
We offer a full refund within 14 days of your first payment if PreRoll isn't the right fit. Contact support@preroll.io.
...
All files uploaded to PreRoll count toward your storage quota: deliverable files, show assets, episode assets, thumbnails, and cover art. External file links (references to Frame.io, Google Drive, etc.) do not count.
...
No. Your data is yours. We don't sell, share, or use your content for any purpose other than providing the PreRoll service.
...
Yes. A single PreRoll account (email) can be a member of multiple organizations. Switch between them using the organization picker in the top navigation.
...
### Can I self-host PreRoll?

Yes. PreRoll supports self-hosting for organizations that need full control over their data and infrastructure.
...
Self-hosted instances use a license key system (soft gate). You can run PreRoll without a license, but a valid key removes the reminder banner and grants access to license-holder support channels.
...
Yes. PreRoll can send webhook events to any URL when things happen (episode created, deliverable approved, episode published, etc.). Use this to connect with:
...
PreRoll works in all modern browsers:
...
Not currently. PreRoll is a web application optimized for desktop use. The client portal is mobile-responsive, so clients can review and approve on their phones.
...
Yes. PreRoll is API-first. Everything the web UI does is available via the REST API. Authenticate with API keys (available on Pro and Studio plans). See the [API documentation](/docs/api) for details.

### Can I use PreRoll with my AI assistant?

Yes. PreRoll includes an MCP (Model Context Protocol) server that AI assistants can use to interact with your production data — creating episodes, checking status, managing deliverables, and more. The MCP server wraps the same REST API that powers the web UI.
